...
using static Kingmaker.UI.Tooltip.DescriptionWindowBase;
using static ToyBox.classes.MainUI.TestMicroLoader;

namespace ToyBox.classes.MainUI {
    internal class TestMicroLoader {
            ...
            protected override void ThreadFunction() {
                ...
                lock (bpCache) {
                    lock (Blueprints) {
                        ...
                        var chunks = SplitList(PreFilter(entries, byteBuffer), 500);
            ...
            private List<BlueprintCacheEntry> PreFilter(List<BlueprintCacheEntry> entries, byte[] byteBuff) {
                ...
                foreach (var entry in entries) {
                    if (entry.Blueprint == null) {
                        ...
                        if (CurrentType != type) {
                            CurrentType = type;
                            Mod.Log($"New Type to load: {CurrentType.Name}");
                        }
                        Filtered.Add(entry);
                        //}
                    }
